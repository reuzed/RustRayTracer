...
use crate::{ray::Ray, sdf::{Sdf, normal}, vec3::{Vec3, unit_vector}};
...
pub fn march(ray: Ray, sdf: impl Sdf) -> MarchResult 
{
    // Take a ray and sdf, march up to some tolerance or max number of steps
        ...
        for i in 0..MAX_STEPS {
            let d = sdf(pos);
            if d < CLOSE_TOLERANCE {
                return MarchResult { hit: true, steps: i, normal: Some(normal(sdf, pos)) };
            }
            pos += unit_dir * d;
        ...
        MarchResult { hit: false, steps: steps_taken, normal: None }
}
